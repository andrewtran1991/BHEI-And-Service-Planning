...
comp_tract_pretty <- combine_words(params$compare_tract)
...
GEOID <- c(params$compare_hhsa, params$compare_sra, params$compare_zcta, params$compare_tract, "San Diego County") 
} else{GEOID <- c(params$compare_hhsa, params$compare_sra, params$compare_zcta, params$compare_tract)
}
...
if(params$separate_tract == FALSE & params$aggregate_tract == TRUE){
   df_separate <- df_separate %>%
  filter(!(compare == "Comparison Area" & type == "Census Tract"))
}   
...
if(params$aggregate_zcta == FALSE  | length(params$compare_zcta) < 2) {
 ...
 if(params$aggregate_tract == FALSE  | length(params$compare_tract) < 2) {
  df_total <- df_total %>%
  filter(!(compare == "Comparison Area" & type == "Census Tract"))
}
...
map <-leaflet() %>%
      addTiles() %>%
      setView(lng = "-116.87", lat="33.0", zoom = 8.5) %>%
      addProviderTiles(provider = "OpenStreetMap") %>%
      addPolygons(data = map_rmd,
                  label = map_rmd$GEOID,
                  color = "#00A9C5",
 opacity = 1.0, fillOpacity = .002, highlightOptions = highlightOptions(color = "white", weight = .5, bringToFront = TRUE))
...
`r if(length(params$compare_tract) >  1 & params$aggregate_tract == TRUE){paste0("* Census Tract Total- Comparison Area:  Data have been aggregated across the following Census Tracts: ", comp_tract_pretty)}`
